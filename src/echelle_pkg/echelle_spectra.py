...
def read_image(fpth, spec="black", crop=[0, -1],crop2=[0, -1], exptime=1):
    """
...
class EchelleImage:
    ...
    def __init__(self, fpth, clbr=None, spec="black", crop=[0, -1],crop2=[0, -1], exptime=1):
...
class Calibrations:
    ...
    folder = join("../resources/calibration_files/20250123")
    dv = 5  # dv*2 + 1 - width of the order in pixels
    filenames = {
        "orders": "pattern.txt",
        "wavelength": "wavelength_hayashi.txt",
    ...
    def make_cutting_masks(self, **kws):
        ...
        for o in ordrs:
            ind = self.cutting_masks[o][0]
            self.cutting_masks[o] = (
                self.cutting_masks[o][0][ind],
                self.cutting_masks[o][1][ind],
            )
    ...
    def wavelength_calibration(self, **kws):
        ...
        w = pd.read_csv(fpth,sep=',',comment='#',names=['ord','from','to','center','wavelength','band'])
